...
* Shows can be exported in Skybrush compiled show format (`.skyc`, preferred), which supports export of trajectories, lights, yaw control, pyro control, cues, audio file, cameras, and even arbitrary terrain objects.
* Simple `.csv` export option of sampled positions and RGB colors with a fixed FPS is available for further post-processing with external tools.
