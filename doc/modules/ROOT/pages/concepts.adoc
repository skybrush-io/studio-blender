...
In this chapter, we explain the basic conventions and the key concepts of *Skybrush Studio for Blender* so you can start designing a drone show from scratch as smoothly as possible.
...
Blender uses a relative coordinate system with _X_, _Y_ and _Z_ axes. The real world has directions of North-South, East-West and Up-Down in geodetic coordinates (latitude, longitude, and altitude).

The mapping from relative to real coordinates is handled by *Skybrush Live* during the setup of your drone show according to the following rules:

1. The origin (0, 0, 0) in Blender will be mapped to a so-called _show origin_ in *Skybrush Live*. This can be moved around freely in your geodetic coordinate system on the map or set to a specific value you prefer.
...
* the minimum distance between any drones at any time during the transition will not be lower than `D/sqrt(2)` (equaling roughly `0.7*D`), assuming that the minimum distance in the source or target formations is `D`.
...
This JSON string gets converted into different visualizations based on the "Render" dropdown selection of the xref:panels/pyro/pyro_control.adoc[Pyro Control panel]. Some visualizations of pyro data need a dedicated material for each drone, this is always stored as the second material of the drone objects.

== Yaw control

*Skybrush* also supports yaw control as part of a drone show for certain drone types. The yaw angle for yaw control in *Skybrush Studio for Blender* is simply inferred from the Z component of the global rotation of objects assuming "XYZ Euler" notation. So far, there are no specific tools to aid yaw control. It is the responsibility of the designer to create a yaw curve that harmonizes with the concept and spatio-temporal trajectory of the show.
...
* Set the "Object Color" and "Wireframe Color" properties of "Viewport Shading" in the top right corner of your 3D Viewport to "Object" as drone colors handled by our addon are sourced from the drone object's viewport color animation data.
...
* In the View tab on the sidebar of the 3D Viewport Editor, set up convenient Clip Start and End values to make your drones visible during the whole show in case you need to render your show
