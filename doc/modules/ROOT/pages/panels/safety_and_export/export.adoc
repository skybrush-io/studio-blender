...
If you are preparing your drone show as part of a larger event, e.g. with fireworks, use our Finale 3D / FWSim .vviz or Depence .ddsf exporter to export your drone show into formats that can be read by these external visualization softwares.

Note that these export options require a professional license that can be purchased from https://shop.skybrush.io[Skybrush Store].
