...
To see the progress or predicted remaining time of the export procedure, run Blender from a terminal window or press "Toggle System Console" in the "Windows" menu to watch its terminal log.
...
Export audio:: Enable this option to export music into the .skyc file for use with Skybrush Viewer. The exporter looks for a single .mp3 audio strip added to Blender's Video Sequence Editor (VSE).
+
NOTE: Audio export is available since Skybrush Studio Server backend version 2.40.0.

Export cameras:: Enable this checkbox to export Blender cameras into the .skyc file (to be used by Skybrush Viewer).

Export terrain (PRO):: Enable this checkbox to export any Blender objects under the persistent "Terrain" scene collection to be embedded into the .skyc file (to be used by Skybrush Viewer).
...
Plot accelerations:: Include acceleration-related plots in the safety report including horizontal and vertical acceleration timelines focusing on maximum acceleration breach warnings.

Plot yaw and yaw rate:: Include yaw-related plots in the safety report including yaw and yaw rate timelines focusing on maximum yaw rate breach warnings.
...
Visit https://shop.skybrush.io[Skybrush Store] to purchase a professional license with third-party exporters. mailto:{contact-email}[Contact us] if you need any additional exporters that are not on the list yet.
...
* External link: https://help.depence.com/depence-construction/drones[Depence: Drones]

WARNING: Even though yaw control and pyro triggering is part of the official .ddsf specification and hence the actual .ddsf files Skybrush exports, according to Depence customer service they only read the trajectories and light program from the .ddsf files so far, not yaw or pyro. Implementation of the latter two is in progress in Depence and we will be notified once it is ready.
