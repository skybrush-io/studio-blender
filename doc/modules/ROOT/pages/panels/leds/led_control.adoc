...
If the material is not an emissive material, Skybrush falls back to adjusting the diffuse color of the material instead.
...
There are several visualization types for LED lights from which you can choose dynamically:

None::
Select this option to skip color visualization completely
(might be needed for speeding up playback or render).

Markers::
Select this option to use very simple but quick color overlay markers on drones. With this option you can still speed up playback and render speed, while having a simple but clear indication of drone colors (including light effects).

Materials:: Select this option to preserve the default coloring scheme through the actual drone objects (this option slows down rendering time on large fleets).

== Marker size

This options defines the size of the LED overlay markers if the visualization type 
is set to Markers. It is hidden otherwise.
...
effect if none of the drones are selected.
...
this behavior is in fact a good approximation of how the LED lights behave on
