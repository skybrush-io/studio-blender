...
If you are still in the motion design phase, we suggest you to uncheck the btn:[Enable light effects] checkbox to avoid slowing down Blender rendering superfluously.
...
You may have noticed that the upper part of the light effects panel is occupied by a list. This list contains the effects themselves. In each frame all the effects are processed in a top-down manner, and for each effect, Blender decides if the effect is going to affect the current frame or not, depending on its temporal and spatial properties. In any given frame, _multiple_ effects may be active, and in this case the original color of each drone (defined by the keyframes of its material) is modified by each active effect in the order defined by the list of effects. This becomes important when the colors of the effects have alpha channels as the output may be a mixture of the original color of the drone as well as one or more active effects.
...
Note that there is no interpolation between pixels in the color image. They are selected for each drone and each frame discretely. Therefore, color images should be used with a width/height that corresponds to the selected output mapping perfectly (e.g., same height as the number of drones, same width as the number of frames in the light effect, etc.), or they should be smooth images without sharp edges to compensate for the rounding errors of the discrete mapping. You can also scale up a low-resolution image in an external image editor tool and use the scaled-up version to simulate interpolation.
...
TIP: For detailed guides on writing your own custom light effects and output functions, see the xref:advanced-topics/index.adoc[Advanced topics] chapter.
...
You can also attach the start and end frames of a light effect to a storyboard entry or transition by selecting the proper entry or transition from the btn:[Attach to] dropdown. If you do so, your start and end frames (and duration) will be kept in synchrony with your selected entry at all times.

TIP: To add an offset to the start or end frames of your light effect relative to the attached storyboard entry start and end times, simply change the start and end times of the light effect while a storyboard entry is attached. You will see the frame offsets next to the start and end frame indicators in brackets (e.g. (+1) or (-5)).
...
selected mesh, and the effect will be applied only to those that are inside.
...
== Drone group constraints

Light effects may also be limited to a single drone group.
This is achieved by selecting a drone group in the *Drone Group* collection dropdown.
...
Gradient (XYZ, XZY, ...):: Drones are sorted according to their coordinates along the X, Y and Z axes (in this order), and they are then distributed evenly along the color ramp. This option has six variants with different axis order.
...
Light preset:: A predefined, hard-coded Python function which returns a number between 0 and 1. All light presets are called for each drone and each frame of the light effect. If you choose this output option, use the btn:[Preset] dropdown appearing below the Light preset setting to choose which light effect preset you wish to use.
+
WARNING: Light effect presets are experimental. The list of available presets or the overall functionality around light presets might change.

Custom expression:: A Python function that returns a number between 0 and 1. The function is called for each drone and each frame of the light effect. An example:
+
```python
...
Custom expression (experimental):: A Python function that updates an array containing the calculated output value for every drone. The function is called for each frame of the light effect and must update the array in a single operation. This allows effects to be expressed as vectorized operations on NumPy arrays and will be more efficient than calling the function once per drone. An example:
+
```python
import numpy as np

def odd_even(effect, context, frame, *, out):
    out[:] = np.arange(len(out)) % 2
```
