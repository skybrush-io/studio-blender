...
Use the btn:[+] button next to the light effect list to add a new light effect to the end of the list. You can remove effects from the list by pressing the btn:[-] button, and you can also rearrange them freely with the btn:[▲] and btn:[▼] buttons. Individual light effects can be enabled or disabled by clicking on the check box in front of the light effect. You can also give a descriptive name to each light effect by double-clicking on its slot to edit the name.
...
then setting the *Target* dropdown to "Inside the mesh". This is a relatively
expensive operation as Blender needs to decide in each frame which drones are
inside the mesh.
