...
Reordering a formation can be issued any time with opening the btn:[Reorder] drop down list and selecting the preferred reordering operator. Note that reordering is instantaneous; using multiple reorder operators is possible, each one will take the actual order and apply its own effect upon that to create the new order. The following reorder operators are implemented:

Sort by name:: sort by the name of items in the formation (default sorting)
Shuffle:: apply a randomization on the order
Reverse:: reverse the actual order
Sort by X/Y/Z coordinate:: sort the actual order according to the X/Y/Z coordinate value of each point in the formation
Every 2nd/3rd/4th:: take every 2nd/3rd/4th item first and repeat until all items are assigned to a new order
Ensure safety distance:: take first item, then take the next that is far enough from it and repeat the process until every item is taken

TIP: to visualize the current order of a formation, enable the btn:[Show order of formations] checkbox, which will connect all points in the formation with linear line segments between each consecutive point in the actual order colored from green to red.
