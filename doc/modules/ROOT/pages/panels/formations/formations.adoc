...
If you wish to have a separate column and row spacing for your basic takeoff grid, enable the btn:[Use separate column spacing] checkbox and setup a separate column spacing.

You can also define a more complex takeoff grid with slots, where each slot contains more than one drone with typically, but not necessarily smaller spacing (see the *Intra-slot* parameters).

TIP: Once the takeoff grid is created, you are free to modify the "Takeoff grid" formation to rearrange the placement of the drones using standard Blender operations, thus you can easily adapt the grid to the requirements of your specific takeoff area. For instance, if there are lampposts, stairs, power lines or any other obstacles in the area, you can move the affected drones to other positions by simply changing the takeoff positions in the "Takeoff grid" formation.

The *Create takeoff grid* operator also creates all Skybrush-specific collections (Drones, Formations, Templates) that you need for your drone show, adds the first "Takeoff grid" entry to the storyboard and initializes your drones in the 3D Viewport editor.
...
If the minimum distance between drones in the takeoff grid is smaller than the spacing parameter of the btn:[Takeoff] operator, a layered takeoff procedure is generated automatically. This procedure ensures that no two drones come closer to each other, either within a layer or between layers, than the specified spacing. Furthermore, the order of the layers is optimized to minimize the effects of drone downwash, making the takeoff procedure as safe as possible.

If you need a greater distance between takeoff layers than the specified takeoff spacing, use the *Layer height* parameter to define an explicit altitude difference between the default layer and any additional target layers above during the layered takeoff procedure.

image::panels/formations/takeoff.jpg[Takeoff]

The btn:[Takeoff] button should ideally be pressed right after the takeoff grid is created, but before any other formations are added. It is also possible to use the operator later after having defined the first few formations, but you must ensure that there is enough time before the first formation to perform the takeoff _and_ get to the first formation in time.
...
It is possible to import static formations or animation snippets into **Skybrush Studio for Blender** from external sources, using the btn:[Generate Markers] button applied to empty formation placeholders created beforehand.

Depending on your external data source, importing formations this way might also create a new baked image-based light effect that is automatically applied on the imported formation to preserve its original color animation.

IMPORTANT: Make sure to append your empty formation to the storyboard before generating markers into it! This ensures that the start and end frames of the imported animation and its color animation will be set up properly.

The following external data sources are available for your new formation:
