...
TIP: if your takeoff place is too small and you need to place your drones closer to each other than their minimum distance between each other in the air, select `Staggered` takeoff to create the proper temporal distance between them during takeoff and land. Use the `Ensure safety distance` reorder operator for the relevant formations.
