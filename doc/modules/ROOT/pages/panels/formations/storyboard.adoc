...
+
The velocity profile of the transition can be selected by the "Profile" setting. Four velocity profiles are supported for each transition: the `Linear` profile keeps a constant speed throughout the entire transition (most probably resulting in a very short acceleration spike at the start and end of the transition); the `Smooth` profile plans an optimal cubic Bézier profile with zero velocity at both edge and a smooth acceleration and deceleration in between; the `Smooth from left` and `Smooth from right` profiles are smooth from left/right and linear on the other side.
+
